...
export default function DownloadDropdown({ initialData, className }: { className?: string, initialData: Record<string, any> }) {
    ...
    return (
        <div ref={dropdownRef} className="relative inline-block text-left">
            ...
            {/* Dropdown */}
            {open && (
                <div className="absolute right-0 mt-2 w-40 p-1 bg-white border border-gray-300 rounded shadow-lg z-50">
                    <button
                        onClick={() => { exportToExcel([initialData.data]) }}
                        className="block w-full text-left text-sm px-4 py-2 hover:bg-gray-100 cursor-pointer"
                    >
                        Export to Excel
                    </button>
                    <PDFDownloadLink
